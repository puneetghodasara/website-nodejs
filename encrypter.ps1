...
gcloud kms encrypt --plaintext-file=keys\config.ini  --ciphertext-file=keys\config.ini.gcp.enc --location=global  --keyring=puneetghodasara  --key=website-key
...
aws kms encrypt --key-id 281f3cd3-9fbe-4ef7-87e5-8029999f81ab --plaintext fileb://keys/config.ini --output text --query CiphertextBlob > .\keys\config.ini.aws.enc.base64
rm keys\config.ini.aws.enc
certutil.exe -decode .\keys\config.ini.aws.enc.base64 .\keys\config.ini.aws.enc
rm keys\config.ini.aws.enc.base64
